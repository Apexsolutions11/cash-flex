...
class CatchCoinsGamePage extends ConsumerStatefulWidget {
  const CatchCoinsGamePage({super.key});

  @override
  ConsumerState<CatchCoinsGamePage> createState() => _CatchCoinsGamePageState();
}

class _CatchCoinsGamePageState extends ConsumerState<CatchCoinsGamePage>
...
class _ResultDialogState extends ConsumerState<_ResultDialog> {
  ...
  Future<void> _claimCoins() async {
    ...
    try {
      ...
      // Even if there's an error, try to award coins
      try {
        final coins = _calculateCoins();
        if (coins > 0) {
          await ApiService.claimCoins(coins, 'CATCH_COINS');
        ...
        debugPrint('Error claiming coins in catch block: $claimError');
      }
      // Restart the game
      if (mounted) {
        Navigator.of(context).pop();
