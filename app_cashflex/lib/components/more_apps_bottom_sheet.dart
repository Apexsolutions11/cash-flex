...
class MoreAppsBottomSheet extends StatelessWidget {
  ...
  List<String> _buildInstructionLines() {
    final template = moreAppsInstructions.trim();
    final lines = template.isEmpty
        ? <String>[
            'Click the button below to open Play Store',
            'Install the app: {name}',
            'Open and use the app for at least {seconds} seconds',
  ...
  Future<void> _launchUrl(BuildContext context, String url) async {
    ...
    if (await canLaunchUrl(uri)) {
      BackgroundTaskGate.instance.start(
        ...
        context: context,
        onSuccess: () async {
          final res = await ApiService.moreAppReward(appId);
